...
const regenerateAccessTokenThroughValidatingRefreshToken = async (res, next, refreshToken) => {
    ...
    try {
        ...
        else if (getValue !== refreshToken) throw { message: 'Refresh Token did not match!!!!' };
...
const validateCurrentAccessToken = async (accessToken) => {
    ...
    if (error) throw error;
    else if (getValue !== accessToken) throw { message: 'Access Token did not match!!!!' };
...
exports.isAuthenticated = async (req, res, next) => {
    ...
    if (req.signedCookies && req.signedCookies.accessToken) {
        ...
        try {
            const { authenticated } = await validateCurrentAccessToken(accessToken, next);
